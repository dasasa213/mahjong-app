...
test('対局編集の検索・並び順・詳細タブ・行追加を確認',async({page})=>{
 ...
 const order=f.locator('#orderInput');await order.evaluate((e:HTMLInputElement)=>e.value='asc');await f.getByRole('button',{name:'検索'}).click();await expect(order).toHaveValue('asc');await shot(page,'対局編集-検索結果');
